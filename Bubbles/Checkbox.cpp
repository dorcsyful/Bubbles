...
{
	m_Sprite = std::make_unique<sf::Sprite>(*a_Texture);
...
bool Checkbox::DetectClick(const sf::Vector2f& a_MousePosition)
{
	if(m_Sprite->getGlobalBounds().contains(a_MousePosition))
	{
		...
		m_Sprite->setTextureRect(sf::IntRect(sf::Vector2i(x / 4 * rect, 0), 
		                                     sf::Vector2i(x / 4, static_cast<int>(m_Sprite->getTexture().getSize().y))));
